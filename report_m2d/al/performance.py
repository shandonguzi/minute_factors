...
def cumulative_returns(returns, period):
    ...
    '''
    改源码，使cumulative_returns可以计算5D、10D等隔天换仓收益率
    '''

    shift = int(re.search(r'\d+', period).group())
    returns_shift = returns.iloc[::shift]

    return ep.cum_returns(returns_shift, starting_value=1)
