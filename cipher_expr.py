...
class Analysis:
    ...
    def __init__(self, 
                 StockData: StockData, 
                 pool_num: int = 2,
                 func_path: str = '/code/minute_factors/report_m2d/config/feature_methods_expr.json') -> None:
    ...
    def loop_cal_factors(self, df):
        ...
        results = []
        for name, function in self.func_config.items():

            result = eval(expr_transform(df.columns, function))
